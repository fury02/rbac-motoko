...
export const RolePermissionsComponent: React.FC = () => {
    ...
    const BindPermissions = async () => {
        if(selected_role != '' && selected_permission != '' && provider == 'Stoic' && agent != undefined && canister_id != undefined){
            const actor = Actor.createActor<service_auth_rbac>(idl_auth_rbac, {
                agent,
                canisterId: Principal.fromText(canister_id)});

            //Action permissions-role bind
            let response = await actor.bind_permission(selected_role, selected_permission);
            if(Object.keys(response)[0] == "ok"){
                ...
                // render(<AlertDialog text_alert={"Error bind permission for role."}/>);
            }
        }
        else if(selected_role != '' && selected_permission != '' && (provider == 'Plug' || provider == 'II') && actor_identity_stored != undefined && canister_id != undefined){
            //Action permissions-role bind
            let response =await actor_identity_stored.bind_permission(selected_role, selected_permission);
            if(Object.keys(response)[0] == "ok"){
                var result = Object.values(response)[0];
                alert("Ok.");
                // render(<AlertDialog text_alert={"Ok."}/>);
            ...
            alert("Not selected permission or role.");
            // render(<AlertDialog text_alert={"Not selected permission or role."}/>);
        }
    }
    ...
    const UnbindPermissions = async () => {
        if(selected_role != '' && selected_permission != '' && provider == 'Stoic' && agent != undefined && canister_id != undefined){
            ...
            //Action permissions-role unbind
            let response = await actor.unbind_permission(selected_role, selected_permission);
            if(Object.keys(response)[0] == "ok"){
                var result = Object.values(response)[0];
                alert("Ok.");
                // render(<AlertDialog text_alert={"Ok."}/>);
                ...
                // render(<AlertDialog text_alert={"Error unbind permission for role."}/>);
            }
        }
        else if(selected_role != '' && selected_permission != '' && (provider == 'Plug' || provider == 'II') && actor_identity_stored != undefined && canister_id != undefined){
            //Action permissions-role unbind
            let response = await actor_identity_stored.unbind_permission(selected_role, selected_permission);
            if(Object.keys(response)[0] == "ok"){
                var result = Object.values(response)[0];
                alert("Ok.");
                // render(<AlertDialog text_alert={"Ok."}/>);
